...
2. **Authentik configuration is IaC, not UI-clicked.** Per-workload Provider, Application, and policy bindings live as a single YAML at `vps-playground/authentik/blueprints/<workload>.yaml` in the platform Authentik repo. The repo bind-mounts `blueprints/` into the Authentik worker at `/blueprints/workloads/`; the worker auto-applies on file-content change. Workload onboarding = "PR a YAML to that repo, push, wait for the redeploy" — no Authentik UI work for Provider/Application/bindings. The single canonical template + per-workload prereq notes (groups must pre-exist, required Provider fields, the silent-rollback failure mode) live in [`blueprints/README.md`](https://github.com/vps-playground/authentik/tree/main/blueprints) of that repo. The one piece that stays a manual UI step is **binding the Application to the embedded outpost** — Authentik blueprints replace rather than append list-typed fields, so codifying it would mean re-listing every existing Application on every workload PR. Operator-side group membership (Directory → Groups, who is in `<workload>-user` / `<workload>-admin`) also stays in the Authentik UI and is intentionally not in git — group membership decisions are per-person, not per-workload, and treating them as code would couple identity decisions to deploy cycles.
