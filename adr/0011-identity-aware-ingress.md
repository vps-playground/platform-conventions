...
2. **Authentik configuration is IaC, not UI-clicked.** Per-workload Provider, Application, and policy bindings live as a single YAML at `vps-playground/authentik/blueprints/<workload>.yaml` in the platform Authentik repo. That repo bind-mounts `blueprints/` into the Authentik worker at `/blueprints/workloads/`; the worker auto-applies on file-content change. Workload onboarding = "PR a YAML to that repo, push, wait for the redeploy" — no Authentik UI work for Provider/Application/bindings. The canonical template, the per-workload prereqs (groups must pre-exist, required Provider fields like `invalidation_flow`), and the silent-rollback failure mode are documented in [`blueprints/README.md`](https://github.com/vps-playground/authentik/tree/main/blueprints) of that repo. Two intentional carve-outs stay manual: (a) **binding the new Application to the embedded outpost** — Authentik blueprints replace rather than append list-typed fields, so codifying this would force re-listing every Application on every workload PR; (b) **group membership** (who is in `<workload>-user` / `<workload>-admin`) stays in the Authentik UI because membership is per-person, not per-workload, and treating it as code would couple identity decisions to deploy cycles.
